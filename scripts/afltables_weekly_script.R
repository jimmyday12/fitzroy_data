...
split_row <- function(row) {
  ...
  if (!is.na(parts_with_dob[1, 1])) {
    # Row with DOB
    return(data.frame(
      Rank = as.integer(str_trim(parts_with_dob[1, 2])),
    ...
    # Row without DOB
    return(data.frame(
      Rank = as.integer(str_trim(parts_without_dob[1, 2])),
...
if (nrow(player_mapping_afltables) == nrow(data_clean)) {
    ...
    tryCatch(
      {
        ...
        number <- as.integer(str_extract(filtered_script, "(?<=document\\.write\\(r\\[)\\d+(?=\\]\\);)"))
  ...
  player_urls_tibble <- player_urls_tibble %>%
    mutate(
      url = case_when(
        rank == 2530 ~ "https://afltables.com/afl/stats/players/J/Jack_Paterson.html", #6712
        rank == 3790 ~ "https://afltables.com/afl/stats/players/J/Jack_Patterson.html", #2060
        rank == 7239 ~ "https://afltables.com/afl/stats/players/R/Robert_Miller1.html", #3362
        rank == 12284 ~ "https://afltables.com/afl/stats/players/C/Charlie_Cameron.html", #12277
        rank == 12584 ~ "https://afltables.com/afl/stats/players/J/Jack_Graham.html", #12576
        rank == 12712 ~ "https://afltables.com/afl/stats/players/J/Jack_Ross.html", #12712
        rank == 12962 ~ "https://afltables.com/afl/stats/players/J/Jack_Williams.html", #12962
        TRUE ~ url # retain the original url if the rank is not in the specified set
      )
    )
  
  loop_df <- player_urls_tibble %>% 
    dplyr::anti_join(player_mapping_afltables, by = c("url")) 
  ...
  player_mapping_afltables <- rows_upsert(player_mapping_afltables, loop_df, by = "rank")
